  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-4 sm:p-6 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        {isMCQ && question.options && (
          <div className="space-y-2">
            {question.options.map((opt: any) => (
              <button key={opt.label} type="button" onClick={() => handleAnswer(opt.label)} className={`flex w-full items-start gap-2 rounded-lg px-3 py-2.5 text-left text-sm transition-colors sm:items-center sm:gap-3 sm:px-4 sm:py-3 ${currentAnswer === opt.label ? "bg-black text-white dark:bg-white dark:text-black" : "border border-zinc-200 text-zinc-700 hover:border-zinc-400 dark:border-zinc-700 dark:text-zinc-300 dark:hover:border-zinc-500"}`}>
                <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold sm:mt-0 ${currentAnswer === opt.label ? "border-white/30 dark:border-black/30" : "border-zinc-300 dark:border-zinc-600"}`}>{opt.label}</span>
        ...
        {isEssay && <div className="mb-3 flex gap-2"><button type="button" onClick={() => setEssayAnswerMode("typed")} className={`rounded-md border px-2.5 py-1.5 text-xs font-medium transition-colors ${essayAnswerMode === "typed" ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black" : "border-zinc-200 text-zinc-600 hover:border-zinc-300 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-zinc-600"}`}>Type</button><button type="button" onClick={() => setEssayAnswerMode("upload")} className={`rounded-md border px-2.5 py-1.5 text-xs font-medium transition-colors ${essayAnswerMode === "upload" ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black" : "border-zinc-200 text-zinc-600 hover:border-zinc-300 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-zinc-600"}`}>Upload</button></div>}
        {isEssay && essayAnswerMode === "typed" && <textarea rows={6} value={currentAnswer} onChange={(e) => handleAnswer(e.target.value)} placeholder="Write your essay response here…" className="w-full resize-none rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm text-black outline-none focus:border-black dark:border-zinc-700 dark:bg-zinc-800 dark:text-white dark:focus:border-white" />}
